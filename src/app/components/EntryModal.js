        ...
        <Modal isOpen={isOpen} onClose={onClose}>
            <form onSubmit={handleSaveClick} className="p-6 space-y-4">
                 <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100">{existingEntry ? "Edit Entry" : "Add Entry"} for {selectedDate?.toLocaleDateString()}</h2>
                    <button type="button" onClick={onClose} className="p-1 rounded-full text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700"><Icon path={ICONS.X} className="w-5 h-5" /></button>
                </div>
                
                <div>
                    <label className="text-sm font-medium text-gray-700 dark:text-gray-300 block mb-1">Claim Type*</label>
                    <select name="claimType" value={formData.claimType || ''} onChange={handleSelectChange} className="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                {formData.claimType === 'Late Finish' && (
                    <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-500/30 rounded-lg space-y-3">
                        <p className="text-sm font-medium text-blue-800 dark:text-blue-200">Overtime Details</p>
                        <div className="grid grid-cols-2 gap-4">
                            <input type="number" name="overtimeHours" value={formData.overtimeHours || '0'} onChange={handleChange} min="0" className="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-gray-900 dark:text-gray-100"/>
                            <input type="number" name="overtimeMinutes" value={formData.overtimeMinutes || '0'} onChange={handleChange} min="0" max="59" className="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-gray-900 dark:text-gray-100"/>
                        </div>
                        <div className="mt-2 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-500/30 rounded-lg text-yellow-800 dark:text-yellow-200 text-xs flex items-start space-x-2">
                    ...
                    <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-500/30 rounded-lg space-y-3">
                        ...
                        {formData.manualMileage ? (
                            <div>
                                ...
                                <input type="number" step="0.1" name="mileage" value={formData.mileage} onChange={handleChange} className="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"/>
                            </div>
                        ) : (
                            <div className="space-y-3">
                                <select name="baseStation" value={formData.baseStation} onChange={handleSelectChange} className="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2">
                                    <option value="">Select Base Station for this journey...</option>
                                    {STATIONS.map(s => <option key={s.name} value={s.name}>{s.name}</option>)}
                                </select>
                                <input type="text" name="destinationPostcode" value={formData.destinationPostcode} onChange={handleChange} placeholder="Destination Postcode" className="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"/>
                            </div>
                        )}
                ...
                {['Late Finish', 'Disturbed Mealbreak', 'Mileage'].includes(formData.claimType) && (
                    <input type="text" name="callsign" value={formData.callsign || ''} onChange={handleChange} maxLength="6" placeholder="Callsign*" className="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-gray-900 dark:text-gray-100"/>
                )}
                ...
                {['Late Finish', 'Disturbed Mealbreak'].includes(formData.claimType) && (
                    <input type="text" name="incidentNumber" value={formData.incidentNumber || ''} onChange={handleChange} placeholder="Incident Number*" className="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-gray-900 dark:text-gray-100"/>
                )}
                {errors.incidentNumber && <p className="text-red-500 text-xs mt-1">{errors.incidentNumber}</p>}

                <textarea name="details" value={formData.details || ''} onChange={handleChange} rows="3" placeholder="Add extra notes..." className="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-gray-900 dark:text-gray-100"></textarea>

                <div className="flex items-center justify-between pt-2">
                    {existingEntry ? 
                        <button type="button" onClick={() => onDelete(existingEntry.id, selectedDate.toISOString().split('T')[0])} className="px-4 py-2 text-sm font-semibold text-red-600 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/20 dark:text-red-400 transition-colors">Delete</button>
                        : <div></div>
                    }
                    <div className="flex items-center space-x-3">
                        <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-semibold text-gray-700 rounded-lg hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700 transition-colors">Cancel</button>
                        <button type="submit" disabled={isCalculating} className="px-5 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors shadow-sm disabled:bg-blue-400 disabled:cursor-not-allowed">
